...
import { ArrowLeft, Camera, FileText, Trash2 } from 'lucide-react-native';
...
export default function CameraToPdfScreen() {
  ...
  const handleConversionProcess = () => {
    ...
    setCustomFileName(`Camera_DocReader_${Date.now()}`);
  ...
  const startConversion = async () => {
    ...
    try {
      ...
      const finalFileName = `${customFileName.trim() || 'Camera_DocReader_Doc'}.pdf`;
      ...
      <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.background }]}>
        <View style={[styles.container, { backgroundColor: theme.background }]}>
          ...
          <View style={[styles.header, { backgroundColor: theme.card, borderBottomColor: theme.border }]}>
            ...
            <Text style={[styles.headerTitle, { color: theme.text }]}>Camera to PDF</Text>
            <View style={{ width: 40 }} />
          </View>

          {images.length === 0 ? (
            <View style={styles.scrollContent}>
              <TouchableOpacity style={[styles.uploadArea, { backgroundColor: isDark ? '#262626' : '#F8FAFC', borderColor: theme.border }]} onPress={capturePhoto} activeOpacity={0.7}>
                <View style={[styles.uploadIconCircle, { backgroundColor: isDark ? 'rgba(239,68,68,0.12)' : '#FEF2F2' }]}>
                  <Camera size={32} color={tintColor} />
                </View>
                <Text style={[styles.uploadText, { color: theme.text }]}>Tap to Capture Photo</Text>
                <Text style={styles.uploadSubtext}>Click one or more photos using camera</Text>
              </TouchableOpacity>
            </View>
          ) : !isSuccess ? (
            <View style={styles.listContainer}>
              <View style={styles.listHeader}>
                <Text style={[styles.sectionTitle, { color: theme.text }]}>{images.length} Photos Captured</Text>
                <TouchableOpacity onPress={capturePhoto}>
                  <Text style={{ color: tintColor, fontWeight: '600' }}>Capture More</Text>
                </TouchableOpacity>
              </View>

              <Text style={styles.dragHint}>Long press and drag to reorder pages</Text>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
              <View style={styles.successContainer}>
                <View style={styles.previewFrame}>
                  ...
                  <Text style={[styles.pdfNameText, { color: theme.text }]} numberOfLines={1}>{customFileName || 'Camera_DocReader_Document'}.pdf</Text>
                </View>
                ...
                >
                  <Text style={{ color: isDark ? '#94A3B8' : '#64748B', fontWeight: '700' }}>Capture Another PDF</Text>
                </TouchableOpacity>
              </View>
            </ScrollView>
          )}
